...
const generateTermoEmprestimo = async (professor, chromebook) => {
  const pdfDoc = await PDFDocument.create();
  const page = pdfDoc.addPage([600, 400]);
  const font = await pdfDoc.embedFont('Helvetica');

  const title = 'Termo de Comprometimento de Empréstimo';
  ...
  const dataEmprestimo = `Data do Empréstimo: ${new Date().toLocaleDateString()}`;
  ...
  page.drawText(dataEmprestimo, { x: 50, y: 240, size: 14, font, color: rgb(0, 0, 0) });
  ...
  link.href = pdfUrl;
  link.download = `Termo_Comprometimento_${professor.masp}_${chromebook.serie}.pdf`;
  link.click();
};

export default generateTermoEmprestimo;
